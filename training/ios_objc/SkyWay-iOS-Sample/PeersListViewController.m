...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (nil != _callback)
	{
		...
		{
            if (YES == [_callback respondsToSelector:@selector(call:)])
			{
				[_callback performSelectorInBackground:@selector(call:) withObject:strTo];
			}
            if (YES == [_callback respondsToSelector:@selector(connect:)])
            {
                [_callback performSelectorInBackground:@selector(connect:) withObject:strTo];
            }
		}];
	}
	else
